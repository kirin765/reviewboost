...
## Guest checkout & social login (extension funnel)

- **Guest checkout**: `/api/billing/checkout` allows anonymous `mode: "redirect" | "overlay"`. Overlay uses Paddle.js `Checkout.open` (`src/components/extension/PaddleCheckoutOverlay.tsx`); redirect mode keeps the existing flow. Guest transactions carry no `user_id`; the webhook resolves the user by customer email (`findUserIdByEmail`, Clerk lookup) and falls back to `pending_subscriptions` when the email has no account yet.
- **Claim**: `claimPendingSubscriptionByEmail(userId, email)` moves `pending_subscriptions` → `subscriptions`+`profiles`; called by `/api/billing/claim` (auto-run on the extension-connect page after login).
- **Naver/Kakao login**: Clerk has no Naver/Kakao OAuth, so `src/lib/social_auth.ts` + `/api/auth/social/[provider]/{start,callback}` implement an OAuth bridge — code exchange → profile → Clerk user lookup/create → **sign-in token** → `/login?__clerk_ticket=...` where the Clerk widget exchanges it into a `__session` cookie (BAPI `sessions.createSession` is dev-only in production). New users get a random `password` because the instance auth config requires it (`password: required` → otherwise `createUser` fails `400 form_data_missing`) — they sign in via Naver/Kakao tickets and can reset the password later. UI buttons in `src/components/Auth/SocialLoginButtons.tsx`. Env: `NAVER_*`, `KAKAO_*` (see `.env.example`).
- **Social button visibility**: `SocialLoginButtons` renders the **Naver** button only when `NEXT_PUBLIC_ENABLE_SOCIAL_LOGIN === "1"`; the **Kakao** button additionally requires `NEXT_PUBLIC_ENABLE_KAKAO_LOGIN === "1"` (Kakao is blocked by review — KOE006 — so production sets only the Naver flag for now; flip Kakao's flag after 검수 passes).
