...
| 11번가 | 미확인 | ❌ 헤드리스 4회 시도 — 상품평 탭 XHR 미발견(리뷰 0개 상품·탭 로드 실패) | 미착수 | **실세션(사용자) 1회**: 리뷰 있는 상품에서 탭 클릭 → XHR 캡처 |
| SSG닷컴 | 미확인 | ⚠️ ajaxItemQnaPageList.ssg?itemId=&page= 캡처됨 — 리뷰 목록 API는 리뷰>0 상품에서 필요 · 헤드리스 검색 실패 | 미착수 | **실세션 1회**: 리뷰 있는 상품 1건 |
