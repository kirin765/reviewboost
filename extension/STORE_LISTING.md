...
2. **개발자 등록** — Chrome Web Store 개발자 미등록 시 1회 $5 결제 (기존 등록자는 불필요).
3. **스크린샷** — 최소 1장(1280×800 또는 640×400) 필요 (기존 3장 재사용 가능).
...
**항목 이름**
```
ReviewBoost 리뷰 수집기 — 쿠팡·스마트스토어·무신사·11번가 등 리뷰 엑셀 내보내기
```

**요약 (간단한 설명, 132자 이내)**
```
지금 보는 쿠팡·스마트스토어·무신사·11번가 등 상품의 리뷰를 엑셀/CSV로 내려받고, ReviewBoost 무료 분석으로 바로 연결하세요. 클릭 한 번, 브라우저 안에서 안전하게.
```
...
검토 거절 시 흔한 사유: 권한 사유 누락(신규 호스트 권한 8개 추가 시 사유 필수), 개인정보처리방침 URL 미도달(배포 필요), 스크린샷 누락 → 위 항목으로 모두 커버됨.