# 웨일 스토어 제출 패키지 — ReviewBoost 리뷰 수집기 (v1.4.1)

업로드 파일: **`extension/reviewboost-extension-whale-1.4.1.zip`** (manifest가 ZIP 루트, v1.4.1)

> ⚠️ 제출은 네이버 아이디 로그인 + (최초 1회) 개발자 등록이 필요해 **직접** 하셔야 합니다. 아래 문구를 그대로 붙여넣으면 됩니다.

> 🆕 v1.4.1 (2026-08-30) — 기존 v1.3.0 항목이 스토어에 있으므로 **"새로운 확장앱 추가"가 아니라 기존 확장앱의 업데이트 제출**로 진행합니다 (확장앱 관리 → 해당 확장앱 → 업데이트 요청).
> 변경 내용: 리뷰 **이미지 URL** 수집 + 내보내기 **스마트스토어 공식 리뷰 엑셀 25열 폼** 채택 + 유료 플랜 **무제한** + **CS 문의 탭** 추가. 상세는 `extension/CHANGELOG.md` 참고.
...
**파일 선택**: `reviewboost-extension-whale-1.4.1.zip`
...
- 보안·개인정보: `activeTab`+단일 상품+선택 전송+CS 문의 입력값 전송만 — 최소 권한 원칙 충족.
...
4. 새 ZIP 업로드 → 등록정보 확인(이미 입력된 값 유지) → 미리보기 확인 → **리뷰 요청**
