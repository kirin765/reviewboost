...
function OverviewMetricBand({
  ...
  return (
    <section className="rounded-[28px] border border-[color:rgba(31,37,89,0.12)] bg-[var(--rb-surface)] p-6 md:p-7">
      <div className="flex flex-col gap-4 border-b border-[color:rgba(31,37,89,0.06)] pb-5 md:flex-row md:items-end md:justify-between">
        <div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-4">
        {metrics.map((item) => (
          <div key={item.label} className="rounded-[18px] border border-[color:rgba(31,37,89,0.06)] bg-[rgba(31,37,89,0.02)] p-4">
...
function TrendSurface({
  ...
  return (
    <section className="rounded-[28px] border border-[color:rgba(31,37,89,0.12)] bg-[var(--rb-surface)] p-6 md:p-7">
      ...
      {points.length > 0 ? (
        <>
          <div className="mt-6 overflow-hidden rounded-[22px] border border-[color:rgba(31,37,89,0.06)] bg-[rgba(31,37,89,0.02)] p-4">
            <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="h-[220px] w-full" role="img" aria-label="최근 분석 부정 비율 추세">
              ...
              {Array.from({ length: 5 }).map((_, index) => {
                ...
                return <line key={index} x1="0" y1={y} x2={chartWidth} y2={y} stroke="rgba(31,37,89,0.05)" strokeDasharray="3 5" />;
              })}
              {Array.from({ length: points.length }).map((_, index) => {
                const x = points.length === 1 ? chartWidth / 2 : (index / (points.length - 1)) * chartWidth;
                return <line key={index} x1={x} y1="0" x2={x} y2={chartHeight} stroke="rgba(31,37,89,0.03)" />;
              })}
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            {points.slice(-3).map((point) => (
              <div key={point.id} className="rounded-[16px] border border-[color:rgba(31,37,89,0.06)] bg-[rgba(31,37,89,0.02)] px-4 py-3">
        ...
        <div className="mt-6 rounded-[22px] border border-[color:rgba(31,37,89,0.06)] bg-[rgba(31,37,89,0.02)] p-6 text-sm leading-7 text-[var(--rb-muted-strong)]">
  ...
  return (
    <section className="rounded-[28px] border border-[color:rgba(31,37,89,0.12)] bg-[var(--rb-surface)] p-6 md:p-7">
      ...
      <div className="mt-6 flex flex-col items-center gap-6 lg:flex-row lg:items-start">
        <div
          className="h-[220px] w-[220px] rounded-full border border-[color:rgba(31,37,89,0.06)]"
          style={{
            background: `conic-gradient(var(--rb-accent) 0deg ${negativeAngle}deg, rgba(31,37,89,0.08) ${negativeAngle}deg 360deg)`
          }}
          aria-hidden="true"
        >
        ...
        <div className="flex-1 space-y-4">
          <div className="rounded-[16px] border border-[color:rgba(31,37,89,0.06)] bg-[rgba(31,37,89,0.02)] p-4">
          ...
          <div className="rounded-[16px] border border-[color:rgba(31,37,89,0.06)] bg-[rgba(31,37,89,0.02)] p-4">
            ...
            <div className="mt-4 h-2 overflow-hidden rounded-full bg-[rgba(31,37,89,0.06)]">
...
function EmptyHomeState({ title, description }: { title: string; description: string }) {
  return (
    <section className="rounded-[28px] border border-[color:rgba(31,37,89,0.12)] bg-[var(--rb-surface)] px-5 py-7 md:px-7">
...
function HomeListSection({
  ...
  return (
    <section className="rounded-[28px] border border-[color:rgba(31,37,89,0.12)] bg-[var(--rb-surface)] px-5 py-6 md:px-7">
      <div className="flex flex-col gap-4 border-b border-[color:rgba(31,37,89,0.08)] pb-5 md:flex-row md:items-end md:justify-between">
        <div>
      ...
      {recentReports.length === 0 ? (
        ...
        <div className="mt-4 overflow-hidden rounded-[18px] border border-[color:rgba(31,37,89,0.06)]">
          <div className="hidden grid-cols-[minmax(0,1.1fr)_120px_120px_120px_100px] gap-3 border-b border-[color:rgba(31,37,89,0.06)] bg-[rgba(31,37,89,0.03)] px-4 py-3 text-xs uppercase tracking-[0.18em] text-[var(--rb-muted)] md:grid">
            <span>파일</span>
            <span>총 리뷰</span>
            <span>부정비율</span>
            <span>우선순위</span>
          ...
          {recentReports.map((report) => (
            <Link
              key={report.id}
              href={report.href}
              className="grid gap-3 border-b border-[color:rgba(31,37,89,0.06)] px-4 py-5 transition last:border-b-0 hover:bg-[rgba(31,37,89,0.02)] md:grid-cols-[minmax(0,1.1fr)_120px_120px_120px_100px] md:items-center"
            >
              <div>
