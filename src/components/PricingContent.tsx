        ...
        <Surface className="px-6 py-7 md:px-8 md:py-9">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {content.pills.map((pill) => (
              <span
                key={pill}
                className="rounded-full border border-[color:rgba(31,37,89,0.08)] bg-[rgba(31,37,89,0.03)] px-3 py-1 text-xs text-[var(--rb-muted-strong)]"
              >
                {pill}
              </span>
            ))}
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {content.plans.map((plan) => {
            ...
            return (
              <Surface
                key={plan.name}
                className={`flex h-full flex-col p-6 md:p-7 ${plan.recommended ? "border-[color:rgba(91,92,234,0.28)] shadow-[0_28px_54px_rgba(34,46,121,0.10)]" : ""}`}
              >
                ...
                <div className="mt-6 flex-1 space-y-3">
                  {plan.features.map((feature) => (
                    <div key={feature.label} className="flex items-center justify-between gap-4 border-b border-[color:rgba(31,37,89,0.06)] pb-3 text-sm last:border-b-0">
