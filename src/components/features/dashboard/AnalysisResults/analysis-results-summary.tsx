...
export function AnalysisResultsSummary({ result, onDownloadPdf, busy, caps, gates }: AnalysisResultsSummaryProps) {
  ...
  return (
    <section className="card resultsSummaryCard">
      ...
      <div className="resultsSummaryHeader">
        <div>
          <p className="sectionEyebrow">Results</p>
          <h2>{result.meta.filename ?? "분석 결과"}</h2>
          <p className="resultsSummaryLead">
            {result.meta.stored ? "저장된 리포트" : "세션 결과"} · {storageEnabled ? caps?.planLabel ?? "계정" : "게스트 모드"}
          </p>
        </div>
      ...
      <div className="resultsSummaryGrid">
        {summaryStats.map((stat) => (
          <article className="appPageMetaItem resultsSummaryStat" key={stat.label}>
