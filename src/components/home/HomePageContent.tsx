...
function EditorialIntro({
  ...
  return (
    <div className="grid gap-6 border-b border-[color:#e6e8f2] pb-8 lg:grid-cols-[minmax(0,0.9fr)_minmax(260px,420px)] lg:items-start">
      <div>
        <p className="text-[11px] uppercase tracking-[0.24em] text-[var(--rb-muted)]">{eyebrow}</p>
        <h2 className="mt-3 max-w-3xl text-[clamp(2.2rem,4vw,4.2rem)] font-semibold leading-[0.96] tracking-[-0.06em] text-[var(--rb-fg)]">
          {title}
        </h2>
      </div>
...
function SummaryRibbon({ metrics }: { metrics: Array<{ label: string; value: string }> }) {
  return (
    <div className="grid gap-4 border-b border-[color:#e6e8f2] pb-5 sm:grid-cols-2 xl:grid-cols-4">
      {metrics.map((metric) => (
        <div key={metric.label}>
          <p className="text-[11px] uppercase tracking-[0.18em] text-[var(--rb-muted)]">{metric.label}</p>
          <strong className="mt-2 block text-[clamp(1.55rem,2vw,2rem)] font-semibold tracking-[-0.05em] text-[var(--rb-fg)]">
            {metric.value}
          </strong>
        </div>
      ))}
    ...
    <div className="space-y-4">
      {categories.map((category, index) => (
        <div key={category.label}>
          ...
          <div className="h-[7px] overflow-hidden rounded-full bg-[#eef0f8]">
            <motion.div
...
function LineChart({
  accent = "rgba(91,92,234,0.95)",
  secondary = "rgba(31,37,64,0.35)"
}: {
  accent?: string;
  secondary?: string;
}) {
    ...
    <svg viewBox="0 0 420 240" className="h-full w-full" aria-hidden="true">
      <defs>
        <linearGradient id={fillId} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="rgba(91,92,234,0.26)" />
          <stop offset="100%" stopColor="rgba(91,92,234,0)" />
        </linearGradient>
      </defs>
      <path d="M0 210H420" stroke="#e6e8f2" strokeWidth="1" />
...
function DonutChart({ share }: { share: number }) {
  return (
    <div
      ...
      style={{
        background: `conic-gradient(var(--rb-accent) 0 ${share}%, #eef0f8 ${share}% 72%, #d5d9ec 72% 100%)`
      }}
      aria-hidden="true"
    >
      <div className="m-auto mt-[18%] flex aspect-square w-[64%] items-center justify-center rounded-full bg-[var(--rb-surface-strong)] text-center">
        <div>
          ...
          <strong className="mt-2 block text-3xl font-semibold tracking-[-0.05em] text-[var(--rb-fg)]">{share}%</strong>
        </div>
      </div>
    </div>
  );
...
function HeroPreview({
  ...
  return (
    <div className="mt-12 rounded-[30px] border border-[color:#e6e8f2] bg-white p-4 shadow-[0_10px_30px_rgba(31,37,64,0.08)] md:p-6 lg:p-8">
      <div className="grid gap-8 lg:grid-cols-[300px_minmax(0,1fr)]">
        <div className="border-b border-[color:#e6e8f2] pb-6 lg:border-b-0 lg:border-r lg:pb-0 lg:pr-8">
          ...
          <div className="mt-8 space-y-5">
            <div className="rounded-[20px] border border-[color:#e6e8f2] bg-white p-4">
              ...
              <div className="mt-4 space-y-4">
                <div>
                  ...
                  <p className="mt-2 text-sm leading-7 text-[var(--rb-fg)]">“받자마자 마감이 벌어져 있어서 바로 반품 고민했습니다.”</p>
                </div>
                <div className="border-t border-[color:#e6e8f2] pt-4">
                  <p className="text-xs text-[var(--rb-muted)]">사용성 · 3일 전</p>
                  <p className="mt-2 text-sm leading-7 text-[var(--rb-fg)]">“설명과 달리 조립이 어렵고 사용법이 직관적이지 않았어요.”</p>
                </div>
              </div>
            </div>
            <div className="rounded-[20px] border border-[color:#e6e8f2] bg-white p-4">
        ...
        <div className="space-y-7">
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,0.92fr)_minmax(220px,0.62fr)]">
            ...
            <div className="rounded-[22px] border border-[color:#e6e8f2] bg-white p-5">
...
function ProductWorkspace({
  ...
  return (
    <div className="rounded-[30px] border border-[color:#e6e8f2] bg-white p-5 md:p-7">
      ...
      <div className="mt-7 border-b border-[color:#e6e8f2] pb-7">
          ...
          {categories.map((category, index) => (
            <div
              ...
              className={`rounded-full border px-4 py-2 text-sm ${
                index === 1
                  ? "border-[color:rgba(91,92,234,0.28)] bg-[rgba(91,92,234,0.1)] text-[var(--rb-fg)]"
                  : "border-[color:#e6e8f2] bg-white text-[var(--rb-muted-strong)]"
              }`}
            >
      ...
      <div className="mt-7 grid gap-8 xl:grid-cols-[minmax(0,1.12fr)_320px]">
        <div className="space-y-7">
          <div>
            <p className="text-[11px] uppercase tracking-[0.2em] text-[var(--rb-muted)]">Urgent reviews</p>
            <div className="mt-4 space-y-4">
              <div className="border-b border-[color:#e6e8f2] pb-4">
                <div className="flex items-center justify-between gap-3 text-xs text-[var(--rb-muted)]">
                  <span>품질</span>
                  <span>impact 9</span>
                </div>
                <p className="mt-3 text-sm leading-7 text-[var(--rb-fg)]">상품 설명과 다르게 품질 편차가 커서 재구매를 망설인다는 리뷰가 반복됩니다.</p>
              </div>
              <div className="border-b border-[color:#e6e8f2] pb-4">
                <div className="flex items-center justify-between gap-3 text-xs text-[var(--rb-muted)]">
                  <span>배송</span>
                ...
                <p className="mt-3 text-sm leading-7 text-[var(--rb-fg)]">파손과 지연이 함께 언급되어 언박싱 만족도가 처음부터 떨어집니다.</p>
              </div>
              <div>
                <div className="flex items-center justify-between gap-3 text-xs text-[var(--rb-muted)]">
                  <span>사용성</span>
                ...
                <p className="mt-3 text-sm leading-7 text-[var(--rb-fg)]">설명서와 실제 사용감의 차이로 설치 난이도가 높게 느껴집니다.</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            {items.map((item) => (
              <div key={item.title} className="border-t border-[color:#e6e8f2] pt-4">
                <p className="text-xs text-[var(--rb-muted)]">{item.stat}</p>
                <h3 className="mt-3 text-lg font-semibold tracking-[-0.03em] text-[var(--rb-fg)]">{item.title}</h3>
        ...
        <div className="border-t border-[color:#e6e8f2] pt-7 xl:border-l xl:border-t-0 xl:pl-8 xl:pt-0">
          ...
          <div className="mt-4 space-y-4">
            <div className="rounded-[18px] border border-[color:#e6e8f2] bg-white p-4">
              ...
              <p className="mt-2 text-sm leading-7 text-[var(--rb-fg)]">상세페이지 첫 화면에서 품질 보증과 실제 소재 설명을 명확히 강화합니다.</p>
            </div>
            <div className="rounded-[18px] border border-[color:#e6e8f2] bg-white p-4">
              <p className="text-xs text-[var(--rb-muted)]">Priority 2</p>
              <p className="mt-2 text-sm leading-7 text-[var(--rb-fg)]">파손/지연 후기에는 배송 포장과 출고 기준을 먼저 공지합니다.</p>
            </div>
            <div className="rounded-[18px] border border-[color:#e6e8f2] bg-white p-4">
              <p className="text-xs text-[var(--rb-muted)]">Simulation</p>
              <p className="mt-2 text-sm leading-7 text-[var(--rb-fg)]">25% 개선 시 3.67, 75% 개선 시 4.67까지 회복 가능합니다.</p>
            </div>
          </div>
        </div>
      </div>
...
function ResultGraph({
  ...
  return (
    <div className="rounded-[30px] border border-[color:#e6e8f2] bg-white p-5 shadow-[0_10px_30px_rgba(31,37,64,0.08)] md:p-7">
      <div className="grid gap-4 lg:grid-cols-[minmax(260px,0.58fr)_minmax(0,1fr)]">
        <div className="rounded-[22px] border border-[color:#e6e8f2] bg-white p-5">
          <p className="text-[11px] uppercase tracking-[0.2em] text-[var(--rb-muted)]">{graph.heroMetricLabel}</p>
          <strong className="mt-3 block text-[clamp(3rem,5vw,4.8rem)] font-semibold leading-none tracking-[-0.08em] text-[var(--rb-fg)]">
            {graph.heroMetricValue}
          </strong>
          <p className="mt-4 max-w-[24ch] text-sm leading-7 text-[var(--rb-muted-strong)]">{graph.heroMetricBody}</p>
        </div>
        <div className="rounded-[22px] border border-[color:#e6e8f2] bg-white p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-[11px] uppercase tracking-[0.2em] text-[var(--rb-muted)]">{graph.lineChartTitle}</p>
              <p className="mt-2 text-sm text-[var(--rb-muted-strong)]">{graph.lineChartCompare}</p>
            </div>
            <span className="text-xs text-[var(--rb-muted)]">0 → 6 months</span>
          </div>
          <div className="mt-4 h-[240px]">
            <LineChart accent="rgba(91,92,234,0.9)" secondary="rgba(31,37,64,0.28)" />
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(260px,0.58fr)_minmax(0,1fr)]">
        <div className="rounded-[22px] border border-[color:#e6e8f2] bg-white p-5">
          <p className="text-[11px] uppercase tracking-[0.2em] text-[var(--rb-muted)]">{graph.donutTitle}</p>
          <p className="mt-2 max-w-[28ch] text-sm leading-7 text-[var(--rb-muted-strong)]">{graph.donutBody}</p>
          <div className="mt-8">
        ...
        <div className="rounded-[22px] border border-[color:#e6e8f2] bg-white p-5">
          <div className="grid gap-5 md:grid-cols-[minmax(180px,220px)_minmax(0,1fr)]">
            <div>
              <p className="text-[11px] uppercase tracking-[0.2em] text-[var(--rb-muted)]">{graph.trendTitle}</p>
              <strong className="mt-3 block text-[clamp(2.6rem,4vw,4rem)] font-semibold leading-none tracking-[-0.06em] text-[var(--rb-fg)]">
                {graph.trendValue}
              </strong>
              <p className="mt-4 max-w-[26ch] text-sm leading-7 text-[var(--rb-muted-strong)]">{graph.trendBody}</p>
            </div>
            <div>
              <div className="mt-2 h-[180px]">
                <svg viewBox="0 0 360 180" className="h-full w-full" aria-hidden="true">
                  <path d="M0 160H360" stroke="rgba(31,37,64,0.1)" strokeWidth="1" />
                  <path d="M0 148L72 148L104 112L168 112L200 74L254 66L308 42L360 24" fill="none" stroke="rgba(31,37,64,0.28)" strokeWidth="2.2" strokeLinecap="round" />
                  <motion.path
                    d="M0 148L72 148L104 112L168 112L200 74L254 66L308 42L360 24"
                    fill="none"
                    stroke="rgba(91,92,234,0.9)"
                    strokeWidth="3"
                    strokeLinecap="round"
                    initial={canObserve ? { pathLength: 0.2, opacity: 0.4 } : false}
                    whileInView={canObserve ? { pathLength: 1, opacity: 1 } : undefined}
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {simulations.map((simulation) => (
                  <div key={simulation.label} className="rounded-[16px] border border-[color:#e6e8f2] bg-[#f4f5fb] p-4">
                    <p className="text-xs text-[var(--rb-muted)]">{simulation.label}</p>
                    <strong className="mt-2 block text-2xl font-semibold tracking-[-0.05em] text-[var(--rb-fg)]">{simulation.value}</strong>
                    <p className="mt-2 text-sm text-[var(--rb-accent)]">{simulation.delta}</p>
                  </div>
                ))}
              </div>
              <div className="mt-4 text-sm text-[var(--rb-muted-strong)]">
    ...
    <div className="pb-12">
      <ShellContainer className="pt-8 md:pt-12">
        <motion.section
          ...
          className="relative overflow-hidden rounded-[36px] border border-[color:#e6e8f2] bg-white px-5 py-10 shadow-[0_10px_30px_rgba(31,37,64,0.08)] md:px-8 md:py-12 lg:px-12 lg:py-14"
        >
          <div className="absolute inset-y-0 left-0 w-[32%] bg-[radial-gradient(circle_at_left,rgba(91,92,234,0.08),transparent_70%)]" aria-hidden="true" />
          <div className="absolute inset-y-0 right-0 w-[28%] bg-[radial-gradient(circle_at_right,rgba(255,122,102,0.06),transparent_70%)]" aria-hidden="true" />

          <div className="relative mx-auto max-w-4xl text-center">
            <p className="text-[11px] uppercase tracking-[0.26em] text-[var(--rb-muted)]">{content.home.hero.eyebrow}</p>
            <h1 className="mt-6 text-[clamp(3.2rem,7vw,6rem)] font-semibold leading-[0.92] tracking-[-0.08em] text-[var(--rb-fg)]">
      ...
      <ShellContainer className="mt-20 space-y-24">
              ...
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1.08fr)_minmax(280px,0.82fr)]">
                ...
                <div className="space-y-6">
                  {content.home.problem.items.map((item) => (
                    <div key={item.title} className="border-t border-[color:#e6e8f2] pt-5">
                      <h3 className="text-2xl font-semibold tracking-[-0.04em] text-[var(--rb-fg)]">{item.title}</h3>
        ...
        <Reveal>
          <section data-home-section="solution" className="space-y-8">
            ...
            />
            <div className="rounded-[30px] border border-[color:#e6e8f2] bg-white p-5 md:p-7">
              <div className="flex flex-wrap items-center gap-3 border-b border-[color:#e6e8f2] pb-6 text-sm text-[var(--rb-muted-strong)]">
              ...
              <div className="mt-7 grid gap-4 xl:grid-cols-4">
                {content.home.solution.strip.map((item) => (
                  <div key={item.title} className="rounded-[24px] border border-[color:#e6e8f2] bg-white p-5">
                    <div className="flex h-24 items-center justify-center rounded-[18px] border border-[color:#e6e8f2] bg-[#f4f5fb]">
                      <span className="rounded-full border border-[color:rgba(91,92,234,0.3)] bg-[rgba(91,92,234,0.1)] px-3 py-1 text-[11px] uppercase tracking-[0.2em] text-[var(--rb-accent)]">
                        {item.meta}
                      </span>
                    </div>
                    <h3 className="mt-5 text-xl font-semibold tracking-[-0.04em] text-[var(--rb-fg)]">{item.title}</h3>
                    <p className="mt-3 text-sm leading-7 text-[var(--rb-muted-strong)]">{item.body}</p>
                  </div>
                ))}
              </div>

              <div className="mt-7 flex flex-wrap gap-x-8 gap-y-3 border-t border-[color:#e6e8f2] pt-6 text-sm text-[var(--rb-muted-strong)]">
        ...
        <Reveal>
          <section data-home-section="pricing" className="space-y-8">
            ...
            />
            <div className="overflow-hidden rounded-[30px] border border-[color:#e6e8f2] bg-white">
              <div className="grid gap-0 lg:grid-cols-3">
                {pricing.plans.map((plan) => (
                  <div
                    key={plan.name}
                    className={`border-b border-[color:#e6e8f2] px-5 py-6 last:border-b-0 lg:border-b-0 lg:border-r last:lg:border-r-0 md:px-7 ${
                      plan.recommended ? "bg-[rgba(91,92,234,0.06)]" : ""
                    }`}
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-[11px] uppercase tracking-[0.22em] text-[var(--rb-muted)]">{plan.label}</p>
                        <h3 className="mt-3 text-3xl font-semibold tracking-[-0.05em] text-[var(--rb-fg)]">
                          {plan.name === "free" ? "무료" : plan.name === "basic" ? "Basic" : "Pro"}
                        </h3>
                      </div>
                      {plan.recommended ? (
                        <span className="rounded-full border border-[color:rgba(91,92,234,0.22)] bg-[rgba(91,92,234,0.1)] px-3 py-1 text-[11px] uppercase tracking-[0.18em] text-[var(--rb-accent)]">
                          추천
                        </span>
                      ) : null}
                    </div>
                    ...
                    <div className="mt-6 space-y-3">
                      {plan.features.map((feature) => (
                        <div key={feature.label} className="flex items-center justify-between gap-4 border-t border-[color:#e6e8f2] pt-3 text-sm">
        ...
        <Reveal>
          <section data-home-section="faq" className="space-y-8">
            ...
            <div className="border-t border-[color:#e6e8f2]">
              {content.home.faq.items.map((item) => (
                <div key={item.question} className="grid gap-4 border-b border-[color:#e6e8f2] py-6 md:grid-cols-[280px_minmax(0,1fr)]">
                  <h3 className="text-lg font-semibold tracking-[-0.03em] text-[var(--rb-fg)]">{item.question}</h3>
              ...
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(260px,360px)] lg:items-center">
                <div className="grid gap-4 sm:grid-cols-3">
                  {[
                    ...
                    <div
                      key={item.title}
                      className="rounded-[22px] border border-[color:#e6e8f2] bg-white p-5"
                    >
                      <h3 className="text-lg font-semibold tracking-[-0.03em] text-[var(--rb-fg)]">{item.title}</h3>
                      <p className="mt-3 text-sm leading-7 text-[var(--rb-muted-strong)]">{item.body}</p>
                    </div>
                  ))}
                </div>
                <div className="flex flex-col gap-3 border-t border-[color:#e6e8f2] pt-6 lg:border-l lg:border-t-0 lg:pl-8 lg:pt-0">
                  <a
                    href={EXTENSION_STORE_URL}
                    target="_blank"
                    rel="noreferrer"
            ...
            <div className="grid gap-4 lg:grid-cols-3">
              {[
                ...
                <Link
                  key={card.href}
                  href={card.href}
                  className="rounded-[26px] border border-[color:#e6e8f2] bg-white p-5 transition hover:border-[color:rgba(91,92,234,0.24)] hover:bg-white md:p-6"
                >
                  <h3 className="text-2xl font-semibold tracking-[-0.04em] text-[var(--rb-fg)]">{card.title}</h3>
        ...
        <Reveal>
          <section data-home-section="cta">
            <div className="rounded-[30px] border border-[color:#e6e8f2] bg-[#eef0f8] px-5 py-7 text-[#111827] shadow-[0_10px_30px_rgba(31,37,64,0.08)] md:px-8 md:py-9">
